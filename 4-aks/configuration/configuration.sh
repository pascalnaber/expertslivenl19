...
az aks get-credentials --resource-group $RESOURCEGROUP_K8S --name $AKS_NAME --overwrite-existing
...
# install ingress
helm upgrade nginxingress --install stable/nginx-ingress --namespace kube-system --set controller.service.loadBalancerIP="$PUBLICIPADDRESS" --set controller.replicaCount=2
# -f internalingress.yaml 
